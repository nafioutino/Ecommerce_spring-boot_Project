...
@RestController // indique que cette classe est un contrôleur REST c'est-à-dire qu'elle gère les requêtes HTTP
...
public class UserController {
    ...
    public ResponseEntity<User> getUserById(@PathVariable Long id) {
        User user = userRepository.findById(id).orElseThrow( // cherche l'utilisateur correspondant à l'id fourni 
            ()->new RuntimeException("User non trouvé")); // renvoie une exception si l'utilisateur n'est pas trouvé
        return ResponseEntity.ok(user); // renvoie l'utilisateur correspondant à l'id fourni
    }

    @PostMapping // indique que cette méthode répond aux requêtes HTTP POST et crée un nouvel utilisateur
    ...
    @PutMapping("/{id}") // indique que cette méthode répond aux requêtes HTTP PUT et met à jour un utilisateur existant
    public ResponseEntity<User> updateUser( @PathVariable Long id, @RequestBody User user) {
        User userToUpdate = userRepository
        .findById(id) // cherche l'utilisateur correspondant à l'id fourni
        .orElseThrow(()->new RuntimeException("User avec l'id " + id + " non trouvé"));  // renvoie une exception si l'utilisateur n'est pas trouvé
    ...
    @DeleteMapping("/{id}") // indique que cette méthode répond aux requêtes HTTP DELETE et supprime un utilisateur existant
    ...
    /**
     * @ResponseEntity: indique que la méthode renverra une réponse HTTP avec un code, c'est-à-dire que c'est une classe qui gère les requêtes HTTP pour les ressources de l'utilisateur
     * @PathVariable: indique que l'id fourni dans l'URL sera utilisé pour identifier l'utilisateur à modifier
     * @RequestBody: indique que l'utilisateur fourni dans la requête HTTP sera utilisé pour modifier l'utilisateur existant c
     */
